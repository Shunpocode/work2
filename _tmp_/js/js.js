...
setInterval(() => {
    if (document.getElementsByTagName('article')[0].getBoundingClientRect().y < 80) {
 ...
 setInterval(() => {
    ...
    if (section[3].getBoundingClientRect().y < 180 && section[3].getBoundingClientRect().y >= -section[3].offsetHeight + 40) {
        coverNav.style.width = navA[2].offsetWidth + 'px';
        coverNav.style.left = navA[2].offsetLeft + 'px';
        if(section[3].getBoundingClientRect().y >= -section[3].offsetHeight + 181){navA[2].style.color = 'white';}else{navA[2].style.color = 'black';}
